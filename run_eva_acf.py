...
n_steps           = 1000                      # update freq for E field
Elow              = 0                         # E strength (low)
Ehigh             = 5e-2                      # E strength (high)
E                 = Ehigh - Elow
Efreq             = 6e14                      # AC E field freq (6e14 Hz)
...
for step in range(0, NVT_2_steps, n_steps):
    # Define global variables once per timestep (outside the nested loops)
    ...
    lmp.command(f"variable     E atom '{E}*sin(2*v_pi*v_freq*(z/3000-v_time))'") # alternating E field - FIXED: changed to atom-style
    ...
    for i in range(n_grid_x):
        for j in range(n_grid_y):
            ...
            lmp.command(f"variable       z_center_{i*n_grid_y+j} delete")
            lmp.command(f"variable       z_min_{i*n_grid_y+j} delete")
            lmp.command(f"variable       z_max_{i*n_grid_y+j} delete")
            # lmp.command(f"variable       efield_down_{i*n_grid_y+j} delete")
    
    # Clean up global variables that are created in each step
    lmp.command("variable       k delete")
    lmp.command("variable       f delete")
    lmp.command("variable       pi delete")
    lmp.command("variable       freq delete")
    lmp.command("variable       time delete")
    lmp.command("variable       E delete")
